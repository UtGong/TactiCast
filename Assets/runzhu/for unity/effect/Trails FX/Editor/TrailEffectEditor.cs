...
namespace TrailsFX {
    ...
    public class TrailEffectEditor : Editor {
        SerializedProperty profile, profileSync, executeInEditMode, targetProp, include, combineMeshesWithChildren, active, ignoreVisibility, ignoreFrames;
        ...
        SerializedProperty renderOrder, clearStencil, hierarchyOccluder, hierarchyOccluderRoot, hierarchyOccluderLayerMask, hierarchyIncludeInactive, cullMode, subMeshMask, cam, lookTarget, lookToCamera, ground;
        ...
        SerializedProperty useLastAnimationState, maxBatches, meshPoolSize, interpolate, animationStates, animator, preserveMultiMaterials;
        ...
        void OnEnable () {
            ...
            combineMeshesWithChildren = serializedObject.FindProperty("combineMeshesWithChildren");
            ...
            hierarchyOccluder = serializedObject.FindProperty("hierarchyOccluder");
            hierarchyOccluderRoot = serializedObject.FindProperty("hierarchyOccluderRoot");
            hierarchyOccluderLayerMask = serializedObject.FindProperty("hierarchyOccluderLayerMask");
            hierarchyIncludeInactive = serializedObject.FindProperty("hierarchyIncludeInactive");
            ...
            preserveMultiMaterials = serializedObject.FindProperty("preserveMultiMaterials");
        ...
        public override void OnInspectorGUI () {
            ...
            switch (effect.intValue) {
                ...
                case (int)TrailStyle.Clone:
                    ...
                    EditorGUILayout.PropertyField(preserveMultiMaterials, new GUIContent("Preserve MultiMaterials"));
                    break;
            ...
            if ((TrailRenderOrder)renderOrder.enumValueIndex == TrailRenderOrder.DrawBehind) {
                EditorGUI.indentLevel++;
                EditorGUILayout.PropertyField(hierarchyOccluder, new GUIContent("Hierarchy Occluder", "Write stencil for every renderer under a hierarchy so the trail always renders behind them."));
                if (hierarchyOccluder.boolValue) {
                    EditorGUILayout.PropertyField(hierarchyOccluderRoot, new GUIContent("Occluder Root", "Leave empty to use the current target."));
                    EditorGUILayout.PropertyField(hierarchyOccluderLayerMask, new GUIContent("Layer Mask"));
                    EditorGUILayout.PropertyField(hierarchyIncludeInactive, new GUIContent("Include Inactive Children"));
                }
                EditorGUI.indentLevel--;
            }
            ...
            EditorGUI.indentLevel++;
            GUI.enabled = thisEffect.isSkinned || thisEffect.isParticle;
            if (thisEffect.isParticle) {
                if (thisEffect.particleRenderer != null && (thisEffect.particleRenderer.renderMode != ParticleSystemRenderMode.Mesh || thisEffect.particleRenderer.mesh == null)) {
                    EditorGUILayout.HelpBox("When using Trails FX on particle systems, best results can be obtained with render mode = mesh or by not using transparent textures for particles.", MessageType.Info);
                }
        ...
        void DrawIncludeArray() {
            include.isExpanded = EditorGUILayout.Foldout(include.isExpanded, new GUIContent("Include", "Additional targets to include in the trail effect. Their trails will be rendered together with the main target."), true);
            
            if (!include.isExpanded) return;
            
            EditorGUI.indentLevel++;
            
            int size = include.arraySize;
            int newSize = EditorGUILayout.IntField("Size", size);
            if (newSize != size) {
                include.arraySize = newSize;
            }
            
            if (include.arraySize > 0) {
                EditorGUILayout.PropertyField(combineMeshesWithChildren, new GUIContent("Combine Meshes", "Combines children meshes with the main mesh and renders as a single trail."));
            }
            
            TrailStyle currentEffect = (TrailStyle)effect.intValue;
            bool supportsOverrides = (currentEffect == TrailStyle.Clone || currentEffect == TrailStyle.Outline) && !combineMeshesWithChildren.boolValue;
            
            for (int i = 0; i < include.arraySize; i++) {
                SerializedProperty element = include.GetArrayElementAtIndex(i);
                ...
                EditorGUILayout.PropertyField(targetProp, new GUIContent("Element " + i));
                
                if (supportsOverrides) {
                    EditorGUILayout.PropertyField(overrideSettings, new GUIContent("Override Settings"));
                    
                    if (overrideSettings.boolValue) {
                        EditorGUI.indentLevel++;
                        
                        if (currentEffect == TrailStyle.Clone) {
                            SerializedProperty luminanceCutOff = element.FindPropertyRelative("luminanceCutOff");
                            EditorGUILayout.PropertyField(luminanceCutOff, new GUIContent("Luminance Cut Off"));
                        }
                        else if (currentEffect == TrailStyle.Outline) {
                            SerializedProperty outlineMethod = element.FindPropertyRelative("outlineMethod");
                            EditorGUILayout.PropertyField(outlineMethod, new GUIContent("Method"));
                            
                            if (outlineMethod.intValue == (int)OutlineMethod.Normals) {
                                SerializedProperty normalThreshold = element.FindPropertyRelative("normalThreshold");
                                EditorGUILayout.PropertyField(normalThreshold, new GUIContent("Normal Threshold"));
                            }
                            else {
                                SerializedProperty rimPower = element.FindPropertyRelative("rimPower");
                                EditorGUILayout.PropertyField(rimPower, new GUIContent("Rim Power"));
                            }
                        }
